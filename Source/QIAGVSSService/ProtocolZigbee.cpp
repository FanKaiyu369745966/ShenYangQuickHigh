...
const unsigned char ProtocolZigbee::g_head = 0xBA;
const unsigned char ProtocolZigbee::g_tail = 0xBE;
const unsigned char ProtocolZigbee::g_encode = 0xB0;

ProtocolZigbee::ProtocolZigbee() :ProtocolBase(ProtocolType::Protocol_Zigbee)
{
}

ByteArray ProtocolZigbee::Create(const ByteArray& data) const
{
	return Create(data.get(), data.size());
}

ByteArray ProtocolZigbee::Create(const unsigned char* data, const int& maxSize) const
{
	...
	//{
	//	qDebug() << QString("%1").arg(source[i]);
	//}

	//qDebug() << endl;

	ByteArray pkt = Encoding(source.get(), index);
...
ByteArrayList ProtocolZigbee::Process(ByteArray& data) const
{
...
ByteArray ProtocolZigbee::Encoding(const unsigned char* data, const int& maxSize) const
{
	int len = 0, transf = 0;
	...
	ByteArray pkt(encode.get(), transf);;
...
ByteArray ProtocolZigbee::Decoding(const unsigned char* data, const int& maxSize) const
{
	int len = 0, transf = 0;
	...
	ByteArray pkt(decode.get(), transf);
