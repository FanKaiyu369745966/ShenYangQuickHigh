...
TaskForm::TaskForm(const QSqlDatabase& db, QWidget* parent) :
	QWidget(parent),
	ui(new Ui::TaskForm),
	m_db(db)
{
	...
	ui->comboBoxTaskDetails->addItem(QString::fromLocal8Bit("取成品入库"));
	ui->comboBoxTaskDetails->addItem(QString::fromLocal8Bit("取成品至热处理车间"));
	ui->comboBoxTaskDetails->setCurrentIndex(-1);

	QSqlQuery query;
	query.exec("SELECT worker_name FROM AGVDB_INFO_WORK");
	while (query.next())
	{
...
{
	m_model->setQuery(QString("SELECT * FROM AGVDB_TASK_CURRENT ORDER BY [task_id] "), m_db);
	...
	m_model->setHeaderData(11, Qt::Horizontal, QString::fromLocal8Bit("备注"));
...
void TaskForm::on_pushButtonDelTask_clicked()
{
	...
	query.prepare(QString::fromLocal8Bit("UPDATE AGVDB_TASK_CURRENT SET [task_status]='取消任务' WHERE [task_id]=%1").arg(strTaskID.toInt()));
	if (!query.exec())
	{
