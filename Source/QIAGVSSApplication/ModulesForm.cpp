...
ModulesForm::ModulesForm(const QSqlDatabase& db, QWidget* parent) :
	QWidget(parent),
...
void ModulesForm::loadModulesData()
{
	...
	m_itemModelAgvType->setQuery(QString("SELECT * FROM AGVDB_INFO_AGVTYPE"), m_db);
	...
	m_itemModelAgv->setQuery(QString("SELECT agv_id,agv_type,agv_client,agv_ip,agv_port,agv_locol_ip,agv_locol_port FROM AGVDB_INFO_AGV"), m_db);
	...
	m_itemModelRestQueue->setQuery(QString("SELECT * FROM AGVDB_INFO_RESTQUEUE"), m_db);
	...
	m_itemModelCaller->setHeaderData(2, Qt::Horizontal, QString::fromLocal8Bit("绑定工作站"));
	...
	m_itemModelWorkStation->setQuery(QString("SELECT worker_id,worker_location,worker_name FROM AGVDB_INFO_WORK"), m_db);
	...
	m_itemModelResetStation->setQuery(QString("SELECT rester_id,rester_location,rester_name,rester_lock FROM AGVDB_INFO_REST"), m_db);
	...
	m_itemModelCharger->setQuery(QString("SELECT charger_id,charger_location,charger_name FROM AGVDB_INFO_CHARGER"), m_db);
	...
	m_itemModelTraffic->setQuery(QString("SELECT * FROM AGVDB_INFO_TRAFFIC"), m_db);
...
void ModulesForm::onUpdateCallerData()
{
	...
	query.exec("SELECT caller_id,caller_address,caller_bind_station,caller_enable FROM AGVDB_INFO_CALLER");
	while (query.next())
	{
